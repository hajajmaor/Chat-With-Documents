...
def process_input(input_type, input_data):
    ...
    if input_type == "Link":
        ...
        texts = [str(doc.page_content)
                 for doc in text_splitter.split_documents(web_documents)]
    elif input_type == "PDF":
...
def main():
    ...
    if input_type == "Link":
        number_of_links = st.number_input(
            "Number of Links", min_value=1, max_value=20, step=1)
        input_data = [st.text_input(f"Link {i+1}")
